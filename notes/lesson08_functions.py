...
print(
    add_item("b")
)  # the list will be created only once, the second call will use the same list created in the first call.
...
def merge_config(
    overrides=None,
    base_config: dict = {"debug": False, "options": {"retries": 3, "timeout": 10}},
):
    """
    把overrides合并进base_config，返回合并后的新配置。
    overrides是一个dict，可能包含顶层key，也可能包含options里的嵌套key。
    """
    ## 无论如何都需要一份深拷贝的base_config字典
